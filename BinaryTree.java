...
public class BinaryTree {
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
        ...
        // if the current node's data matches the old value, replace it with the new value
        if (node.data == oldVal) {
            node.data = newVal;
        }
    ...
    private int findMinHelper(Node node) {
        ...
        // get the minimum value in the left and right subtrees
        int leftMin = findMinHelper(node.left);
        int rightMin = findMinHelper(node.right);

        // the min value is the smallest between the current node's data, left subtree, and right subtree
    ...
    private int nodesGTHelper(Node node, int val) {
        ...
        // count the current node if it's greater than val
        int count = 0;
        if (node.data > val) {
            count = 1;
        }

        // recursively count nodes greater than val in the left and right subtrees
    ...
    private int[] averageHelper(Node n) {
        // if the current node is null, return {0, 0} for both sum and count
        if (n == null) {
            return new int[] {0, 0};
        }

        // recursively calculate the sum and count from the left and right subtrees
